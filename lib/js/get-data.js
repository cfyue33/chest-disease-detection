...
FHIR.oauth2.ready().then(function(client) {
  ...
  client.request("Observation?" + query, {
    ...
    function(ob) {
      ...
      p.weight = weight[0] ? getQuantityValueAndUnit(weight[0]) : getQuantityValueAndUnit(weight2[0]);
    ...
    var query = new URLSearchParams();
    query.set("patient", client.patient.id);
    client.request("MedicationRequest?" + query).then(
      function(meds) {
  ...
  //var data = {
    ...
    //text: annotation
  //};
  var authorString = "Test Author";
  var currTime = date.toISOString();
  displayText = authorString + ' - ' + currTime + ' - ' + annotation;
  displayAnnotation(displayText);
}

//event listner when the add button is clicked to call the function that will add the note to the weight observation
 document.getElementById('add').addEventListener('click', addImageAnnotation);
